...
export default function TransactionDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(`${term('operationalRecord')} deleted successfully`);
      // Navigate back to list
      window.location.href = '/app/transactions';
    } catch (error: any) {
      toast.error(error.message || `Failed to delete ${term('operationalRecord').toLowerCase()}`);
    }
  };
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (isFirstPosted) {
        ...
        toast.success(`Posted successfully. ${term('operationalRecord')} ID: ${result.id}`);
      }
      setShowPostModal(false);
    } catch (error: any) {
      toast.error(error.message || `Failed to post ${term('operationalRecord').toLowerCase()}`);
    }
  };
  ...
  if (!transaction) {
    return <div data-testid="transaction-detail">{term('operationalRecord')} not found</div>;
  }
    ...
    <div data-testid="transaction-detail">
      <div className="mb-6">
        <Link to="/app/transactions" className="text-[#1F6F5C] hover:text-[#1a5a4a] mb-2 inline-block">
          ← Back to {term('operationalRecordPlural')}
        </Link>
        <h1 className="text-2xl font-bold text-gray-900 mt-2">{term('operationalRecord')} details</h1>
        {id && (
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <dt className="text-sm font-medium text-gray-500">{term('operationalRecord')} date</dt>
      ...
      {transaction.correction_posting_group_id && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
          <p className="text-sm text-green-800">
            Corrected by <Term k="postingGroup" showHint />{' '}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${term('operationalRecord')}`}
        message={`Are you sure you want to delete this ${term('operationalRecord').toLowerCase()}? This action cannot be undone.`}
        variant="danger"
      />
    </div>
  );
