...
export default function LabourOwedPage() {
  const { data: rows = [], isLoading } = usePayablesOutstanding();
  const navigate = useNavigate();
  const { formatMoney } = useFormatting();

  const withBalance = rows.filter((r) => parseFloat(r.payable_balance) > 0);
  ...
  return (
    <div className="max-w-2xl mx-auto pb-24 sm:pb-6">
      ...
      />
      <p className="text-sm text-gray-500 mb-4">From outstanding wages.</p>
      ...
      {withBalance.length === 0 ? (
        ...
        <ul className="space-y-2 mb-6">
          {withBalance.map((r) => (
            <li key={r.worker_id}>
              <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm flex flex-wrap items-center justify-between gap-3">
                <div>
