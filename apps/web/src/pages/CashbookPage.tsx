...
export default function CashbookPage() {
  ...
  const handleExport = () => {
    const mapped = data.map((row) => ({
      date: exportDateIsoYmd(row.date),
      description: row.description,
      reference: row.reference,
      type: row.type,
      amount: exportAmountForSpreadsheet(parseFloat(row.amount || '0')),
    }));
            ...
            {data.length > 0 && (
              <div className="p-4 bg-gray-50 border-t totals-row">
                <div className="grid grid-cols-4 gap-4 text-sm font-medium">
                  ...
                  <div>Transactions: {formatNumber(data.length)}</div>
                </div>
              </div>
            )}
          </div>
          ...
          >
            <table className="w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-4 text-center text-gray-500">{EMPTY_COPY.noDataForPeriod}</td>
                  </tr>
                ) : (
                  data.map((row, i) => (
