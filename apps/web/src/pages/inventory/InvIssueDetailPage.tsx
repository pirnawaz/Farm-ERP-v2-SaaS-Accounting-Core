...
export default function InvIssueDetailPage() {
  ...
  const canConfirmIssueReverse = Boolean(id && reversePostingDate && reverseReason.trim());

  const handleReverse = async () => {
    if (!canConfirmIssueReverse) return;
    try {
  ...
  if (!issue) return <div>Stock used record not found.</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      {isDraft && canEdit ? (
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <FormField label={term('activities')}><input value={activity_id} onChange={(e) => setActivityId(e.target.value)} className="w-full px-3 py-2 border rounded" /></FormField>
            {machineryEnabled && (
      ...
      <Modal
        ...
        title={`Record to accounts: ${term('issue')}`}
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-700 leading-relaxed">
            This will reduce stock in the store and add cost to the field cycle (and related accounts) for the posting date below. Cancel if you need to edit the draft first.
          </p>
          <PrePostChecklist
                ...
                if (issue.allocation_mode === 'HARI_ONLY') {
                  return (
                    <div className="text-sm space-y-1">
                      <div>Total value: <span className="font-medium">{formatMoney(totalValue)}</span></div>
      ...
      <Modal
        ...
        title={`${term('reverseAction')}: ${term('issue')}`}
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-700 leading-relaxed">
            This creates offsetting stock and cost entries as of the posting date below. Cancel if you are not ready.
          </p>
          <PrePostChecklist
            items={[
            ...
            blockingHint={!canConfirmIssueReverse ? 'Choose a posting date and enter a reason before reversing.' : undefined}
          />
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 pt-4">
            ...
            <button
              ...
              disabled={!canConfirmIssueReverse || reverseM.isPending}
