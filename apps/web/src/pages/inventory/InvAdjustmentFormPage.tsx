              ...
              <div key={i} className="border border-gray-200 rounded-lg p-4 bg-gray-50/50 space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField label="Item">
                    <select value={line.item_id} onChange={(e) => updateLine(i, { item_id: e.target.value })} disabled={!canEdit} className="w-full px-2 py-1 border rounded text-sm"><option value="">Select item</option>{items?.map((it) => <option key={it.id} value={it.id}>{it.name} ({it.uom?.code})</option>)}</select>
                  </FormField>
                  <FormField label="Qty delta">
                    <input type="number" step="any" value={line.qty_delta} onChange={(e) => updateLine(i, { qty_delta: e.target.value })} disabled={!canEdit} placeholder="-2 or 3" className="w-full px-2 py-1 border rounded text-sm" />
                  </FormField>
                </div>
                {canEdit && <div className="flex justify-end"><button type="button" onClick={() => removeLine(i)} className="text-red-600 text-sm">Remove</button></div>}
              </div>
            ))}
          </div>
        </div>
