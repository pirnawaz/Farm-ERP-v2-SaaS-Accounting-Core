...
export function FarmSnapshotCard({ farm, scope }: FarmSnapshotCardProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
      ...
      <p className="text-xs text-gray-500 mb-4">Crop cycle: {scope.label}</p>
      <dl className="grid grid-cols-2 gap-3 text-sm">
        <div>
          ...
          <dd className="font-semibold text-gray-900 tabular-nums">
            <Link to="/app/crop-cycles" className="text-[#1F6F5C] hover:underline">
              {formatNumber(farm.active_crop_cycles_count)}
            </Link>
          </dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="font-semibold text-gray-900 tabular-nums">{formatNumber(farm.harvests_this_cycle_count)}</dd>
        </div>
        <div>
          <dt className="text-gray-500">Unposted records</dt>
          <dd className="font-semibold text-gray-900 tabular-nums">{formatNumber(farm.unposted_records_count)}</dd>
        </div>
      </dl>
    </div>
  );
