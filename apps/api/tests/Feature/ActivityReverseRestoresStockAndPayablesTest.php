...
class ActivityReverseRestoresStockAndPayablesTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_reverse_activity_restores_stock_and_payables(): void
    {
        TenantContext::clear();
        ...
        $rev = $this->withHeader('X-Tenant-Id', $tenant->id)->withHeader('X-User-Role', 'accountant')
            ->postJson("/api/v1/crop-ops/activities/{$activityId}/reverse", ['posting_date' => '2024-06-16', 'reason' => 'Correction']);
        $rev->assertStatus(201);

        $activity = CropActivity::find($activityId);
        $this->assertEquals('REVERSED', $activity->status);
        $this->assertNotNull($activity->reversed_at);
        ...
        $activity->refresh();
        $reversalPg = PostingGroup::where('reversal_of_posting_group_id', $activity->posting_group_id)->first();
        $this->assertNotNull($reversalPg);

        $reversingMov = InvStockMovement::where('tenant_id', $tenant->id)->where('posting_group_id', $reversalPg->id)->first();
        $this->assertNotNull($reversingMov);
        $this->assertEquals(2, (float) $reversingMov->qty_delta);

        $workerBal = LabWorkerBalance::where('tenant_id', $tenant->id)->where('worker_id', $worker->id)->first();
        $this->assertNotNull($workerBal);
        $this->assertEquals(0, (float) $workerBal->payable_balance);
    }
}
