...
class ActivityPostFailsWhenInsufficientStockTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_post_activity_fails_when_insufficient_stock(): void
    {
        TenantContext::clear();
        ...
        InvGrnLine::create(['tenant_id' => $tenant->id, 'grn_id' => $grn->id, 'item_id' => $item->id, 'qty' => 1, 'unit_cost' => 50, 'line_total' => 50]);
        ...
        $cr = $this->withHeader('X-Tenant-Id', $tenant->id)->withHeader('X-User-Role', 'accountant')
            ->postJson('/api/v1/crop-ops/activities', [
                ...
                'labour' => [['worker_id' => $worker->id, 'units' => 1, 'rate' => 100]],
            ]);
        $cr->assertStatus(201);
        ...
        $this->assertNull($activity->posting_group_id);

        $pgForActivity = PostingGroup::where('tenant_id', $tenant->id)->where('source_type', 'CROP_ACTIVITY')->where('source_id', $activityId)->first();
        $this->assertNull($pgForActivity);

        $movCount = InvStockMovement::where('tenant_id', $tenant->id)->where('source_type', 'crop_activity')->where('source_id', $activityId)->count();
        $this->assertEquals(0, $movCount);
    }
}
