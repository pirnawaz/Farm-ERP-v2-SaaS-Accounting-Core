 ...
 * Uses the pack snapshot (latest version snapshot_json) only; no recalculation.
 */
class SettlementPackPdfRenderer
{
    ...
    public function render(SettlementPack $pack, int $documentVersion, string $sha256Short = ''): string
    {
        ...
        $asOf = $tb['as_of'] ?? $bs['meta']['as_of'] ?? $pack->as_of_date?->format('Y-m-d') ?? $pack->prepared_at?->format('Y-m-d') ?? '';
        $from = $pl['meta']['from'] ?? '';
...
<tr><td><strong>Prepared</strong></td><td>{$pack->prepared_at?->format('Y-m-d H:i')}</td></tr>
